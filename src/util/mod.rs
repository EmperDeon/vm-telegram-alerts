...
pub fn formatted_elapsed(to: DateTime) -> String {
  let from = chrono::Utc::now().with_nanosecond(0).unwrap_or_else(chrono::Utc::now);
  ...
  duration = duration
    .checked_sub(&chrono::Duration::weeks(duration.num_weeks()))
    .unwrap();

  result += &localize(duration.num_days(), "", "1 day ", "days ");
  duration = duration
    .checked_sub(&chrono::Duration::days(duration.num_days()))
    .unwrap();

  result += &localize(duration.num_hours(), "", "1 hour ", "hours ");
  duration = duration
    .checked_sub(&chrono::Duration::hours(duration.num_hours()))
    .unwrap();

  result += &localize(duration.num_minutes(), "", "1 minute ", "minutes ");
  duration = duration
    .checked_sub(&chrono::Duration::minutes(duration.num_minutes()))
    .unwrap();
